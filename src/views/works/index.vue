  ...
  <div class="container">
    <a-row  class="row-main">
      <a-col class="!flex justify-center" :xs="24" :sm="24" :md="12" :lg="8" :xl="8" v-for="(item, index) in workList" :key="item.title">
        ...
        >
          <div class="cover">
            <img @click="showMore(item)" alt="example" :src="item.cover" />
          </div>
          <div class="content">
            <a-typography>
              <a-typography-title class="!text-lg mt-2">{{ item.title }}</a-typography-title>
              <a-typography-paragraph style="color: #697b8c">{{ item.desc }}</a-typography-paragraph>
            </a-typography>
            <div class="action">{{ item.time }}</div>
          </div>
        </div></a-col
      >
    </a-row>
